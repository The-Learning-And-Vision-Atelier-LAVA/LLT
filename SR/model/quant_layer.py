...
class QuantConv2d(nn.Module):
    def __init__(self, channels_in, channels_out, kernel_size=3, stride=1, padding=None, bias=False, w_bits=4, a_bits=4, sign=True):
        ...
        self.act_quant = quant_lookup(self.granu, a_bits, sign) if self.a_bits < 32 else None
    ...
    def forward(self, x):
        if self.training:
            ...
            kernel_q = self.kernel_quant(self.weight) if self.w_bits < 32 else x
